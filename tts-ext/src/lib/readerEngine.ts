...
export class ReaderEngine {
 ...
 private async onWorker(data:any,worker:Worker,epoch:number){
  ...
  else if(data.type==='split'&&data.index===this.index){if(!Array.isArray(data.parts)||data.parts.join('')!==this.chunks[this.index]||data.parts.some((s:string)=>!s.length))throw Error('Invalid speech split.');clearTimeout(this.watchdog);this.busy=false;if(this.sourceChunks.length)this.sourceChunks.splice(this.index,1,...splitSourceChunk(this.sourceChunks[this.index],data.parts));this.chunks.splice(this.index,1,...data.parts);this.metrics.tokenSplits=(this.metrics.tokenSplits??0)+1;this.emit({totalChunks:this.chunks.length});this.pump();}
  else if(data.type==='audio'&&data.index===this.index&&this.stream){
   clearTimeout(this.watchdog);const samples=data.samples as Float32Array;if(!samples.length||!Number.isFinite(data.sampleRate)||data.sampleRate<=0||samples.some(x=>!Number.isFinite(x)))throw Error('The voice returned invalid audio.');
   const stream=this.stream;if(this.sourceChunks.length){const audioStart=this.cueEnd;this.cueEnd+=samples.length/data.sampleRate;this.cues.push({...this.sourceChunks[this.index],audioStart,audioEnd:this.cueEnd});}this.index++;this.metrics.firstPcmMs??=performance.now()-this.metrics.requestStartedAt;this.metrics.chunks.push({index:data.index,chars:this.chunks[data.index].length,audioSec:samples.length/data.sampleRate,generationMs:data.generationMs,padding:data.padding,arrivedMs:performance.now()-this.metrics.requestStartedAt});
 ...
 private async onNativeMessage(message:NativeHostResponse,epoch:number){
  ...
  if(message.type==='ended'||message.type==='cancelled'){
   const userStopped=this.state.stopping||message.type==='cancelled';this.nativeRequestId=null;this.done=true;
   if(!userStopped)this.metrics.completeMs=performance.now()-this.metrics.requestStartedAt;
   this.emit({phase:'complete',stopping:false,stopReason:userStopped?'user':undefined,spokenPosition:{precision:'unavailable'},message:userStopped?'Stopped. Replay starts from the beginning.':'Finished. Replay starts from the beginning.',modelResident:false,generatedChunks:userStopped?undefined:1});
  }
 }
 ...
 private tick(epoch:number){
  ...
  this.cues=this.cues.filter(cue=>cue.audioEnd>start);
