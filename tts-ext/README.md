...
Version **2.1.1** moves Kokoro to ONNX Runtime 1.26.0’s native WebGPU implementation, retaining the original full-precision model and voices; see the [measured diagnosis and review status](../docs/evidence/kokoro-freeze/REVIEW.md). Version 2.1.0 added **Mac voice (Start Speaking)**; [build/install instructions and verification boundaries](../native/mac/README.md). Its controls are Stop and replay from the beginning. Kokoro retains its seek, pause, speed and bounded audio retention.

After building this update, **Reload Local Reader** on Chrome's extensions page, verify version **2.1.1**, then **refresh the article tab** to discard its older injected controller. Existing Chrome shortcut assignments remain unchanged; use the popup buttons if another extension currently owns a shortcut.
...
The build applies a narrowly scoped, SHA-256-guarded compatibility patch to the pinned ONNX Runtime bundle in `scripts/ort-webgpu-submit.ts`. Its GPU dispatch batch changes from 16 to 1 so released buffers return to the existing bounded freelists earlier. An upstream runtime change fails the build until the patch is reviewed. Model weights, voice data, chunking, inference precision and speech speed are unchanged. Worker-owned tensor wrappers are also disposed after extracting the CPU waveform. That cleanup alone did not explain the GPU peak.
